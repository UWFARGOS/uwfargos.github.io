...
def classify(job, employer_kind="other", max_years=1):
    """
    Decide whether a posting belongs on the board and tag it.

    Returns the job dict with `keep`, `reject_reason`, `level`, `bucket`,
    and `flags` added. Nothing is silently dropped — rejects keep their
    reason so you can audit what the filter is throwing away.
    """
